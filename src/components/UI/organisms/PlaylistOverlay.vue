...
<template>
  <Transition>
    <div
      v-if="isClicked"
      class="absolute top-0 left-0 w-screen h-screen bg-gray-900/50 flex items-center justify-center px-5 py-5 z-[999]"
      @click.self="$emit('closeOverlay')"
    >
      <div
        class="grid grid-rows-[1fr_2fr] background-overlay rounded-3xl shadow-xl w-full h-full overflow-hidden"
